...
function testCategoryName($tst_ref)
{
    include 'fabinde.php';
    $returner = "";

    $categoryNames = mysqli_query($con,"SELECT * FROM test_categories WHERE test_ref='$tst_ref'");
    if(mysqli_num_rows($categoryNames) >= 1)
    {
        while($res = mysqli_fetch_assoc($categoryNames))
        $returner = $res['test_category'];
    }
    return $returner;
}
...
function numberOfTestParameters($tst_ref)
{
    ...
    if (mysqli_num_rows($test_query) >= 1) {
        while ($rest = mysqli_fetch_assoc($test_query)) {
            if($rest['test_parameters'] != "none" && !empty($rest['test_parameters']))
            {
                
                $number_of_paramters = unserialize($rest['test_parameters']);
            }
            else
            {
