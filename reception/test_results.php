...
{
    foreach($result_array as $checker)
    {
        $patientReferenceNo_status = "";
        $checking_result1 = mysqli_query($con,"SELECT * FROM test_results WHERE test_result_ref='$checker'");
...
if(is_array($patientReferenceNo))
{
    foreach($patientReferenceNo as $updatingRef)
    {
        $updates = "finish";
